 ...
 *
 * Three stages, each its own card: import a public Threads post (or paste it), adapt it into
 * Hebrew with the translation agent, then preview and export. The deck it produces is a
 * TechTipDeck, so rendering, the 9:16 reel and the ZIP export are the exact same code paths the
 * Tech Tips studio uses — this tab adds a source, not a second pipeline.
 *
 ...
 */
export default function ThreadsImporter() {
  const [url, setUrl] = useState('');
  const [rawText, setRawText] = useState('');
  const [showPaste, setShowPaste] = useState(false);
  const [thread, setThread] = useState<ImportedThread>(EMPTY_THREAD);
  ...
  useEffect(() => {
    if (!restored.current) return;
    if (!thread.posts.length && !deck) return;
    saveThreadState({ thread, deck, url, notes, savedAt: Date.now() });
  }, [thread, deck, url, notes]);
  ...
  /** Stage 1 — pull the thread off Threads. A gated post is not an error: the paste box opens. */
  const runImport = useCallback(async () => {
    const target = url.trim();
    if (!isThreadsUrl(target)) {
      setImportError('הדביקו קישור לפוסט ב-Threads (threads.com/@user/post/…).');
      return;
    }
    setImporting(true);
    setImportError(null);
    try {
      ...
      setShowPaste(true);
      setImportError((e as Error).message || 'ייבוא השרשור נכשל — הדביקו את הטקסט ידנית.');
    } finally {
      setImporting(false);
    }
  }, [url]);
  ...
  const applyPaste = useCallback(() => {
    const parsed = parseThreadRawText(rawText, url);
    setThread(parsed);
  ...
  const generate = useCallback(async () => {
    ...
    resetOutputs();
    try {
      const d = await synthesizeThreadDeck(source, notes);
      setDeck(d);
      await renderDeck(d);
    } catch (e) {
  ...
  }, [thread, rawText, url, notes, resetOutputs, renderDeck]);

  /** Swap backgrounds and re-render in the selected style — the adapted copy is untouched. */
  ...
  const downloadVideo = useCallback(() => {
    ...
    const slug = (deck.title || 'thread').replace(/[^\w֐-׿]+/g, '-').slice(0, 40) || 'thread';
    const objectUrl = URL.createObjectURL(videoBlob);
    const a = document.createElement('a');
    a.href = objectUrl;
    a.download = `mrdaniel-thread-${slug}.mp4`;
      ...
      <div className="dash-card p-6">
        <div className="flex items-center justify-between flex-wrap gap-3 mb-4">
          <span className="flex items-center gap-2 text-zinc-400 text-xs font-mono uppercase tracking-wider">
            <AtSign className="w-3.5 h-3.5" /> ייבוא שרשור מ-Threads
          </span>
          {(thread.posts.length > 0 || deck) && (
            <button
              onClick={clearAll}
        ...
        <div className="flex flex-wrap gap-2">
          <div className="relative min-w-[18rem] flex-1">
            ...
            <input
              ...
              placeholder="https://www.threads.com/@username/post/ABC123"
              dir="ltr"
        ...
        {showPaste && (
          <div className="mt-3 rounded-lg border border-white/10 bg-black/30 p-3">
            <label className="mb-1.5 block text-[12px] font-bold text-zinc-300">
              הדביקו את טקסט השרשור (פוסט לכל פסקה, או שורות ממוספרות "1/", "2/")
            </label>
            <textarea
              value={rawText}
              ...
              rows={8}
              dir="auto"
              placeholder={'1/ Most teams get RAG wrong…\n\n2/ Here is what actually works…'}
        ...
        {/* extracted posts — the operator sees exactly what will be adapted */}
        ...
        <PreviewErrorBoundary label="תצוגת הקרוסלה" resetKeys={[deck.createdAt, images.length, active]} onReset={() => setActive(0)}>
          <div className="dash-card p-6">
            ...
            <QuickPublishBar
              ...
              label="פרסום מהיר · קרוסלה מ-Threads"
