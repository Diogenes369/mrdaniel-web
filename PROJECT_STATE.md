...
| RTDB rules (target) | **Written, NOT yet published (2026-09-24): `database.rules.json`.** Everything admin-only (`admins/<uid>` = true, or verified `daniel@mrdaniel.co.il`); browsers keep create-only `events`, own `presence/<id>`, `health/latest`. Visitor accounts (free-guide sign-in) get no RTDB access: their lead goes through `api/leads` `guide-signup`. **Service-account blocker replaced by a service USER**: `server-agent@mrdaniel.co.il` (uid `jLXHUOCybfYloHqSFc4uKCnAeB42`), creds in `FIREBASE_SERVER_EMAIL`/`FIREBASE_SERVER_PASSWORD` (root `.env` + site Vercel Production/Preview); `getServerDb()` and the MCP worker sign in with it. Order: deploy -> in the console add `admins/<service uid>: true` and `admins/<owner uid>: true` -> publish the rules. The PM2 worker needs a restart to pick up the creds. |
