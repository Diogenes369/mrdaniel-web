...
| RTDB rules (target) | **Written, NOT applied (2026-09-24): `database.rules.json`.** Everything admin-only (`admins/<uid>` = true, or verified `daniel@mrdaniel.co.il`); browsers keep create-only `events`, own `presence/<id>`, `health/latest`. Visitor accounts (free-guide sign-in) get no RTDB access: their lead goes through `api/leads` `guide-signup`. **Blocked on `FIREBASE_SERVICE_ACCOUNT`**: every server write (leads, newsletter, snapshots, agent_queue, ...) still uses the anonymous client, so publishing these rules first breaks them. Order: key on the site project -> redeploy -> seed `admins/<owner uid>` -> publish the rules in the console. |
