...
const TOPIC_HASHTAGS: Record<NewsTopic, string[]> = {
  ...
  devops: ['#DevOps', '#SysAdmin', '#CI_CD', '#GitOps', '#Kubernetes', '#אוטומציה', '#ITOps', '#Observability'],
...
const HOOKS: Record<NewsTopic, string[]> = {
  cyber: [
    '{title} — וזו בדיוק סוג התזכורת ששווה להבין לעומק אם התחום הזה מעניין אתכם.',
  ...
  devops: [
    '{title}. הצוותים הכי אמינים הם אלה שאף פעם לא סומכים על "זה עבד אצלי".',
    'דיפלוי הוא לא רגע חד-פעמי, הוא תהליך שצריך לשרוד 3 בלילה בלי אף אחד ער. {title}',
    '{title} — עוד תזכורת שאוטומציה בלי ניטור היא רק דרך מהירה יותר לשבור דברים.',
    'התשתית שאף אחד לא מדבר עליה היא זו שקובעת אם התקלה הבאה תיפתר בדקות או תהפוך לאירוע. {title}',
  ],
  general: [
...
const WHY_IT_MATTERS: Record<NewsTopic, string> = {
  ...
  devops:
    'מעבר לכותרת: DevOps אמיתי הוא לא כלי, הוא איך צוותי פיתוח ותשתיות עובדים ביחד — פייפליין CI/CD שרץ אוטומטית, תשתית-כקוד שגרסאתית ובת-שחזור, וניטור שמתריע לפני שהלקוח מרגיש. ההבדל בין "יש לנו DevOps" לבין הרצת סקריפטים הוא משמעת: בדיקות אוטומטיות בכל דיפלוי, Rollback מוגדר מראש, ותרבות של תחקור תקלות (Postmortem) בלי חיפוש אשמים.',
  general:
...
const TAKEAWAYS: Record<NewsTopic, string[]> = {
  ...
  devops: [
    'פייפליין CI/CD עם בדיקות אוטומטיות בכל שלב — כך תקלה נתפסת לפני הפרודקשן, לא אחריו.',
    'Rollback מוגדר מראש (לא מאולתר תחת לחץ) הוא ההבדל בין דיפלוי כושל לבין אירוע לילה שלם.',
    'ניטור ו-Observability (מטריקות, לוגים, Tracing) שמתריעים לפני שהלקוח פותח קריאת שירות.',
    'GitOps — כל שינוי בתשתית עובר קוד-ריוויו וגרסאות, כמו כל שינוי בקוד האפליקציה.',
    'תרבות של Postmortem בלי חיפוש אשמים היא מה שהופך תקלה חוזרת לתקלה חד-פעמית.',
    'On-call מוגדר עם Runbooks ברורים — כדי שתגובה לאירוע לא תלויה במי שזוכר איך תוקן בפעם הקודמת.',
  ],
  general: [
...
const EXPERT_INSIGHT: Record<NewsTopic, string> = {
  ...
  ai_models:
    'מהשטח: הארגונים שמפיקים הכי הרבה ערך ממודלים חדשים הם אלה שבנו שכבת הפשטה (Abstraction) מעל ספק המודל — כך שמעבר מ-GPT ל-Claude ל-Gemini, או משדרוג גרסה, הוא שינוי קונפיגורציה ולא כתיבה מחדש. מי שחיבר את המודל ישירות לכל שכבות המוצר משלם על כך בכל עדכון.',
  cloud:
  ...
  devops:
    'מהשטח: הצוותים שסופגים הכי פחות אירועים לילה הם לא אלה עם הכי הרבה כלים, אלא אלה שמריצים אותו פייפליין דיפלוי — עם אותן בדיקות — לכל שינוי, קטן כגדול. ברגע שיש נתיב "מהיר" שעוקף בדיקות "כי זה דחוף", הוא הופך לנתיב הכי נפוץ, ואז לתקלה הכי יקרה.',
  general:
...
const TECH_CONTEXT: Record<NewsTopic, string> = {
  ...
  devops:
    'הקשר הטכני: פייפליין CI/CD בריא הוא Build → בדיקות אוטומטיות (יחידה, אינטגרציה, אבטחה) → דיפלוי הדרגתי (Canary / Blue-Green) → ניטור פעיל עם Rollback אוטומטי במקרה חריגה. GitOps שומר את הגדרת התשתית בגרסאות לצד הקוד, ו-Observability (מטריקות + לוגים + Tracing) הוא מה שמקצר את הזמן מ"משהו לא בסדר" ל"מצאנו את השורש".',
  general:
...
const TOPIC_FALLBACK_IMAGE: Record<NewsTopic, string> = {
  cyber: 'https://images.pexels.com/photos/5380664/pexels-photo-5380664.jpeg?auto=compress&cs=tinysrgb&w=1080',
  ...
  cloud: 'https://images.pexels.com/photos/1148820/pexels-photo-1148820.jpeg?auto=compress&cs=tinysrgb&w=1080',
  devops: 'https://images.pexels.com/photos/1181675/pexels-photo-1181675.jpeg?auto=compress&cs=tinysrgb&w=1080',
