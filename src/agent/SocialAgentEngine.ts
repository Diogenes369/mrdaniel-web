...
// because the generators below and igGrowthStrategy.ts import it from this module.
export { AUDIENCE_RULES } from './expertVoice.js';
...
/**
 * Engagement contract for article-derived posts.
 *
 * The synthesis prompts already enforced accuracy and completeness, which produced correct posts
 * that read like a wire report and drew no comments. This block is the other half of the brief:
 * the post has to open on a hook that earns the second line, has to say what the article means for
 * the reader's own business, and has to close on a question a reader can actually answer from
 * experience. It is deliberately additive — every grounding rule in the host prompt still wins on
 * conflict, so "provoke discussion" can never license inventing a fact to argue about.
 */
export const ENGAGEMENT_RULES = `הנעה למעורבות — הפוסט נמדד בתגובות, לא רק בדיוק:
...
3. הסיום — קריאה לשיח, לא לצריכה:
   ...
   - אסור: שאלה שאפשר לענות עליה בכן/לא, שאלה גנרית ("מה דעתכם?", "מעניין, לא?"), או בקשה ישירה לתגובות/שיתופים ("תגיבו למטה", "שתפו את הפוסט").
   - שאלה אחת בלבד, בשורה נפרדת, בסוף — לא שרשרת שאלות ולא שאלה באמצע הפוסט.

4. קריאוּת — פוסט שלא נסרק לא נקרא:
...
- טון: בטוח, ישיר, טכני-אך-נגיש, עם דעה. לא "היי חברים!", לא סופרלטיבים ריקים.
...
- רגיסטר אחיד: אל תערבב עברית ספרותית-גבוהה עם עברית עממית-שיחתית באותו טקסט — הרגיסטר הוא של איש מקצוע בכיר מדבר עם עמית: בהיר, מדויק, ז'רגון מקצועי במקום שבו אנשי מקצוע באמת משתמשים בו, בלי סלנג רחוב ובלי שפה גבוהה מדי, ולשמור עליו לאורך כל השקפים.

${EXPERT_VOICE_RULES}`;

/**
 * Post shape — three beats, and a hard stop after the third.
 *
 * Added 2026-09-20, when the paid tier removed the pacing delay and it became cheap to read a lot
 * of generated posts in a row. The voice rules were holding (no "בעידן הדיגיטלי"), but the shape
 * was not: the model spent its first paragraph explaining the topic to a reader who had already
 * decided to read, and its last apologising for stopping. Correct Hebrew, zero tension.
 *
 * So the structure is now named and numbered rather than implied. "Short and punchy" is a style
 * note the model agrees with and then ignores; "hook, one payload, one CTA, nothing else" is a
 * shape it can actually check its own draft against — the same reason shortCaptionRules() states
 * its limits numerically.
 *
 * Scoped to the single-post path (CONTENT_SYSTEM_INSTRUCTION) on purpose. It is NOT in
 * EXPERT_VOICE_RULES, which is shared with the slide generators: a carousel body slide is a
 * deliberate 50–70 word paragraph and a three-beat rule would fight the layout it is written for.
 */
export const POST_SHAPE_RULES = `מבנה הפוסט — שלושה חלקים, ואז עוצרים:

1. הוק — משפט אחד, לכל היותר שניים. הדבר החד ביותר שיש: המספר, הסתירה, הטעות שהקורא כנראה עושה עכשיו. לא הקדמה, לא הגדרה של הנושא, לא "כולנו יודעים ש". מי שמתחיל ברקע כבר איבד את הקורא.
2. הבשר — מה קרה ואיך זה עובד. זה החלק היחיד שמותר לו להיות יותר ממשפט: 2-4 משפטים, עובדה ואז ההסבר שלה — המנגנון, הסיבה, או מה זה מלמד על התחום. נקודה אחת מוסברת טוב מנצחת שלוש נקודות מוזכרות.
3. סגירה — משפט אחד. שאלה שאפשר לענות עליה מניסיון, או צעד אחד קונקרטי. לא שניהם, לא רשימה, ולא משפט סיכום שחוזר על מה שכבר נאמר.

אורך: זו תקרה, לא יעד. פוסט קצר יותר שעושה את העבודה עדיף תמיד.
- LinkedIn: עד 130 מילה.
- Instagram: עד 90 מילה.
- TikTok: עד 60 מילה.

קצב:
- משפטים קצרים. פסקה לא עולה על 3 משפטים, ושורה ריקה בין פסקה לפסקה — פוסט שנראה כמו קיר טקסט לא נקרא.
- משפט אחד בשורה משלו הוא ההדגשה החזקה ביותר שיש בערוץ שלא מרנדר Markdown. השתמש בזה פעם אחת, במקום הנכון.
- אם משפט לא מוסיף עובדה, משמעות או צעד — הוא נמחק. זה נכון במיוחד למשפט הראשון ולמשפט האחרון, ששם נולדת רוב שפת ה-AI.`;
...
- התובנה חייבת להיות טכנית ואמיתית, לא תיאור שטחי של הנושא.`
        : // Word counts match POST_SHAPE_RULES in the system instruction; they were 200/120 until
          // the 2026-09-20 tone pass, and two different ceilings in one prompt is how a model ends
          // up choosing the looser one. Change both or neither.
          `כתוב פוסט בודד המתאים ל-${
            platform === 'linkedin' ? 'LinkedIn (טון מקצועי בגובה העיניים, עד 130 מילה, פסקאות קצרות עם שורות ריקות ביניהן לקריאות)' : platform === 'tiktok' ? 'כיתוב TikTok (קצר וקולע, עד 60 מילה, hook חד בשורה הראשונה)' : "Instagram (טון נגיש יותר, עד 90 מילה, אפשר אימוג'י אחד אם הוא מוסיף)"
          } לפי שלושת החלקים: הוק, הבשר, סגירה אחת. בלי פסקת רקע פותחת ובלי משפט סיכום.`;
...
- כל פסקת תוכן מלמדת לפחות דבר אחד קונקרטי מתוך המקור — מנגנון, מונח, מספר או הסבר למה משהו עובד ככה — בתוך הפסקה, לא כרשימה.
...
2. השקופיות באמצע (8–10) עוקבות אחרי סדר הרעיונות בשרשור: kind:"concept" להסבר רעיון, kind:"step" לשלב בתהליך (עם stepNumber רץ 1,2,3…), kind:"tool" לרשימת כלים, kind:"code" רק אם השרשור עצמו הכיל קוד, kind:"takeaway" לשורה התחתונה.
...
2. כותרת נפתחת בפועל או במספר, לא במילת קישור: "ככה בונים סוכן ב-4 דקות", ולא "על האופן שבו ניתן לבנות סוכן".
3. פנייה ישירה בגוף שני רבים ("תפתחו", "תדביקו", "שימו לב"). בלי סביל ובלי "המשתמש".
4. אסורות מילות מילוי: "בעולם של היום", "חשוב לציין", "ניתן לומר", "בעידן ה-AI", "בואו נצלול", "לסיכום". מוחקים אותן, לא מחליפים במילה אחרת.
...
3. כל שאר השקופיות: kind:"concept" להסבר רעיון, kind:"step" לשלב בתהליך ממוספר (עם stepNumber התואם למה שמופיע בפועל על השקופית), kind:"tool" לרשימת כלים, kind:"code" אם השקופית מציגה קוד/פקודת טרמינל/קובץ הגדרות, kind:"takeaway" לשורה התחתונה.
...
- "translatedText": תרגום עברי שוטף וטבעי לבלוק הזה בלבד (לא לכל השקופית) — אם boxType הוא "watermark" תוכלו להשאיר את זה ריק, זה יוחלף בקוד. חריג קריטי: אם boxType הוא "command_code" — פקודת סלאש, קוד, נתיב קובץ או פקודת טרמינל — translatedText הוא בדיוק originalText, אות-באות, ללא שינוי; אסור לתרגם אותו לעברית. היוצא מן הכלל היחיד בתוך command_code: משתנה בסוגריים מרובעים (כמו [topic]) כן מתורגם בתוך הסוגריים (למשל [topic] -> [נושא]), אך שאר הפקודה (הסלאש, שם הפקודה, הסוגריים עצמם) נשארת באנגלית מדויקת.
