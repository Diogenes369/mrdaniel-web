...
2. הזווית העסקית — למה זה נוגע לקורא:
   - כל פוסט חייב לענות במפורש על "ומה זה אומר עליי": בעל עסק, עצמאי או יוצר תוכן בישראל שמנהל את העסק בעצמו.
   - התרגום חייב להישאר נאמן לכתבה. אם הכתבה עוסקת בתאגיד ענק — הסבר איזה עיקרון ממנה חל גם על עסק של שלושה אנשים, בלי להמציא נתון על עסקים קטנים שלא הופיע בה.
...
const IG_FORMAT_SPEC = `פורמט אינסטגרם — בנוי לסריקה מהירה בפיד:
   ...
   • קריאה לפעולה: שאלה פתוחה אחת, קונקרטית ונגזרת מהכתבה, שהקורא יכול לענות עליה מהניסיון שלו בעסק (לא שאלת כן/לא ולא "מה דעתכם?"), ואחריה הפניה לקישור בביו.
...
2. מבנה: שורת הוק חדה אחת שעוצרת גלילה — ספציפית לכתבה, לא ביטוי גנרי ("מאבק משפטי חדש", "בעולם של היום", "התפתחות מעניינת"). אחריה 2-3 פסקאות קצרות (2-3 משפטים כל אחת) שמוסרות את העובדות המהותיות ואת המשמעות המעשית לבעל עסק, לעצמאי או ליוצר תוכן, וכוללות חיבור טבעי אחד לשירותים של דניאל (סוכני AI, אוטומציה לעסק). לסיום שורה אחת עם שאלה פתוחה שנובעת מהכתבה ושחבר קהילה יכול לענות עליה מהניסיון שלו — לא שאלת כן/לא, לא "מה דעתכם?" ולא בקשה לשתף.
