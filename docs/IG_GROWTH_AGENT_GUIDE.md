...
## 5. מנוע אסטרטגיית צמיחה אורגנית (IG Growth Strategy Engine)

שכבה שנייה, מעל התוכן שכבר נוצר — לא טאב נפרד אלא פאנל **"ציון צמיחה אורגנית והמלצות"** (`GrowthScorePanel.tsx`) שמותקן מתחת לתצוגת הקרוסלה והריל ב"מחולל תוכן מחדשות", ב"מחולל סטורי" וב"יבוא ושכתוב תוכן".

### 5.1 Hook ושימור — בתוך פרומפטי הסינתזה
- `HOOK_RETENTION_RULES` (הפרעת דפוס ויזואלית + משפט חד, 6 תבניות: number / contrarian / risk / result / question / myth, מבחן ההחלפה, שקופית 2 משלמת מיד, לולאה פתוחה) ו-`SAVE_SHARE_RULES` (צ'קליסט, workflow, פרומפטים, לפני/אחרי, טריגר שליחה) ב-`SocialAgentEngine.ts`.
- נכנסים לפרומפטים של הקרוסלה (STORY_SYNTH — בגרסה מותאמת לפסקאות בלבד), סטודיו הקרוסלות, הריל ותסריט הווידאו של הטייס האוטומטי.
- `story-synthesize` ו-`reel-script-synthesize` מחזירים גם `hookOptions` — 3 פתיחים ל-3 השניות הראשונות (`line` + `visual` + `pattern`). השדה תוספתי; לקוחות ישנים מתעלמים ממנו.

### 5.2 הציון — מקומי, מיידי, מוסבר (`growthScore.ts`)
| ממד | משקל | מה נמדד |
|---|---|---|
| עוצמת ה-Hook | 35% | אורך (4–12 מילים), מספר, סתירה, סיכון, שאלה, פנייה ישירה, מונח ספציפי; קלישאות מורידות |
| פוטנציאל שמירה | 30% | פעלי פעולה, מבנה צעדים, שקופית צ'יט-שיט, פרומפט/קוד, קריאה לשמור, טריגר שליחה, צפיפות ואורך |
| טריגר תגובה | 20% | CTA מגנט לידים בכיתוב, מילת טריגר תקינה, הודעת DM, שאלה פתוחה; engagement bait מוריד |
| חיפוש ו-SEO | 15% | 3–5 האשטגים, עוגני נישה ישראליים, תג באנגלית, שורת מילות מפתח, מילת מפתח ב-125 התווים הראשונים, OCR בריל |

אין קריאת AI בחישוב הציון. AI רץ רק בלחיצה על אחד מהכפתורים.

### 5.3 כפתורי שיפור בלחיצה (`/api/agent-generate · action:"growth-optimize"`)
- **חדד את ה-Hook** (`op:"hooks"`) — 3 חלופות חדשות; תוצאת AI מוחלת אוטומטית על השער/הפתיח, חלופות מקומיות (משפטים מתוך הטקסט) מוצגות לבחירה ידנית בלבד.
- **הוסף שקופית צ'יט-שיט / סצנת סיכום** (`op:"cheat-sheet"`) — פסקה אחת עם 3–5 צעדים ממוספרים מתוך התוכן, נכנסת לפני ה-CTA (`insertContentSlide`, בכפוף למקסימום 6 שקופיות תוכן).
- **צור CTA מגנט לידים** / **חבילת האשטגים ו-SEO** (`op:"pack"`) — קריאה אחת שמחזירה את שניהם.

### 5.4 Comment-to-DM ו-ManyChat
- **טריגר מעורבות** — מתג (כבוי כברירת מחדל, נשמר ב-`localStorage['ig:engagement_trigger_v1']`), מילת טריגר, מה נשלח בפרטי, קישור לכפתור ה-DM.
- מילת הטריגר: מילה אחת, 2–14 תווים, בלי מירכאות/אימוג'י/**סימני כיווניות** (RLM בתוך המילה הופך אותה לבלתי ניתנת להקלדה), לא מילה נפוצה בתגובות.
- הפלט: שורת CTA לכיתוב, רשימת מילות מפתח ל-ManyChat (כולל צורת ה-, אותיות גדולות/קטנות, מקבילה באנגלית), 3 תשובות פומביות מתחלפות, הודעת DM + כפתור, ו"העתק הגדרת ManyChat" — גיליון הגדרה מוכן להדבקה.
- כשהמתג דלוק, ה-CTA נכנס לכיתוב ושאלת הסיום הגנרית של התבנית יוצאת — CTA אחד ברור.

### 5.5 האשטגים ו-SEO
- עד 5 תגים: 3 עוגני נישה בעברית (למשל `#בינהמלאכותית #סייבר #אוטומציה #פיתוחתוכנה`) + 2 תגים באנגלית בנפח גבוה.
- שורת מילות מפתח טבעית ("🔎 עוד על … — ב-mrdaniel.co.il") **מתחת לקיפול "…עוד"**, אחרי שתי שורות ריווח של U+2800. זה טקסט גלוי לכל מי שפותח את הכיתוב — לא טקסט מוסתר, שהוא טקטיקת black-hat ולא נקלט בחיפוש ממילא.

### 5.6 מה אסור (white-hat)
engagement bait ("תייגו 3 חברים", "תגיבו כן אם"), הבטחה בלי משאב אמיתי מאחוריה, keyword stuffing, וכל מספר שלא מופיע בתוכן המקור.
